...
type LandingPricingAudience = 'general' | 'business';
type LandingBillingCycle = 'monthly' | 'annual';
...
export class PublicWikisComponent implements OnInit {
  ...
  readonly activePricingAudience = signal<LandingPricingAudience>('general');
  readonly pricingBillingCycle = signal<LandingBillingCycle>('monthly');
  ...
  readonly pricingPlans: LandingPricingPlan[] = [
    {
      audience: 'general',
      name: 'Reader',
  ...
  readonly activePricingCopy = computed(() =>
    this.activePricingAudience() === 'business'
      ? {
  ...
  readonly activePricingPlans = computed(() => {
    const annual = this.pricingBillingCycle() === 'annual';
    return this.pricingPlans
      .filter((plan) => plan.audience === this.activePricingAudience())
      .map((plan) => ({
        ...plan,
  ...
  readonly upgradePromptTitle = computed(() =>
    this.isSignedIn()
      ? 'Upgrade your LivingWiki account'
      : 'Upgrade when you are ready for more than browsing',
  );
  readonly upgradePromptDescription = computed(() =>
    this.isSignedIn()
      ? 'You are not on a paid pricing plan yet. Pick a personal or business tier below to unlock private spaces, richer publishing, or local business tools.'
  ...
  setPricingAudience(audience: LandingPricingAudience): void {
    this.activePricingAudience.set(audience);
  }

  setPricingBillingCycle(cycle: LandingBillingCycle): void {
    this.pricingBillingCycle.set(cycle);
  }
